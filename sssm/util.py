...
DATETIME_PRECISION_TYPES = {
    "time",
    "datetime2",
    "datetimeoffset",
}  # 1 parameter (datetime precision)
CHAR_TYPES = {"char", "varchar", "nchar", "nvarchar", "binary", "varbinary"}


def column_sql_representation(
    name,
    data_type,
    identity=False,
    nullable=False,
...
def data_type_representation(
    ...
    if data_type in INT_TYPES | MONEY_TYPES | DATETIME_NO_PRECISION_TYPES:
        ...
        if numeric_precision is None:
            raise ValueError(
                "Numeric precision must be provided for types: {}".format(NUMERIC_TYPES | APPROX_NUMBER_TYPES)
            )
